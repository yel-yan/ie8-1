...
class GWModal extends React.Component {
  ...
  handleUpload = (info) => {
    ...
    if (info.file.status === 'done') {
      ...
      if(info.file.response.code == 0 && info.file.response.data.Number == 1062){
        ...
        axios.post('/file/get_file_info',param,{baseURL:'/img',headers: { 'Content-Type': 'multipart/form-data', Accept: 'text/html' },}).then((data) => {
          console.log(data)
          let upload = {};
  ...
  handleSubmit = (e) => {
    ...
    API.POST('signing/addto',formData).then((data) => {
        console.log('添加公文')
        if(data.code == 1){
            message.success(data.msg);
            this.props.form.resetFields();
